...
import { Copy, Check, Volume2, ThumbsUp, ThumbsDown, Share, Download } from "lucide-react";
...
export function ChatMessage({ message }: { message: ChatMessageType }) {
  ...
  const handleSpeak = () => {
    if ('speechSynthesis' in window) {
      const utterance = new SpeechSynthesisUtterance(message.content.replace(/\[GENERATED_IMAGE\]/g, ''));
      utterance.rate = 1;
      utterance.pitch = 1;
  ...
  const isImageMessage = message.content.includes("[GENERATED_IMAGE]");
  const imageMatch = message.content.match(/!\[.*?\]\((data:image\/[^)]+)\)/);
  const imageUrl = imageMatch?.[1];
  const textAfterImage = message.content
  ...
  const handleDownloadImage = () => {
    ...
    a.href = imageUrl;
    a.download = `mindibly-image-${Date.now()}.png`;
    a.click();
  };
      ...
      <div className={`${isUser ? "max-w-[80%]" : "max-w-full"} min-w-0`}>
        {isUser ? (
          ...
          <div className="w-full">
            <div className="rounded-2xl overflow-hidden border border-border max-w-lg">
              <img src={imageUrl} alt="Generated by Mindibly" className="w-full h-auto" />
            </div>
            {textAfterImage && (
              <p className="text-[15px] text-muted-foreground mt-2">{textAfterImage}</p>
            )}
            <div className="flex items-center gap-1.5 mt-2.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={handleDownloadImage} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Download">
          ...
          <div className="w-full">
            ...
            <div className="flex items-center gap-1.5 mt-2.5 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Like">
                <ThumbsUp className="h-4 w-4" />
              </button>
              <button className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Dislike">
