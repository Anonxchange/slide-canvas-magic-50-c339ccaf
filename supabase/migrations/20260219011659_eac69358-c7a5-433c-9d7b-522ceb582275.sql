...
-- Add user_id column to conversations (nullable for guest sessions)
ALTER TABLE public.conversations ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Add user_id column to messages (nullable for guest sessions)
...
-- New conversation policies: logged-in users see their own, guests see nothing persisted
